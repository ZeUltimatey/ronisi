    ...
    <header className="sticky top-0 z-50 border-b border-white/10 bg-slate-950/95 shadow-lg shadow-black/20 backdrop-blur-xl">
      <div className="mx-auto flex min-h-20 max-w-[90rem] items-center justify-between gap-5 px-4 lg:px-8">
        ...
        <nav className="hidden items-center gap-1 lg:flex" aria-label={t('mainNavigation')}>
          ...
          </div> : <Link key={group.href} href={group.href} className={`rounded-lg px-3 py-2 text-sm font-bold transition ${isCurrent(group.href) ? 'bg-cyan-300/15 text-cyan-300' : 'text-white/80 hover:bg-white/5 hover:text-white'}`}>{getLabel(group)}</Link>)}
        </nav>
      </div>
      {open && <nav className="max-h-[calc(100vh-5rem)] overflow-y-auto border-t border-white/10 px-4 py-3 lg:hidden" aria-label={t('mobileNavigation')}>
        {navGroups.map(group => <div key={group.label.lv} className="py-1">
          {group.href ? <Link href={group.href} className={`block rounded-xl px-4 py-3 font-bold ${isCurrent(group.href) ? 'bg-cyan-300 text-slate-950' : 'hover:bg-white/10'}`}>{getLabel(group)}</Link> : <details open={group.items.some(item => isCurrent(item.href))} className="rounded-xl bg-white/[.03] px-4 py-3"><summary className="cursor-pointer font-bold">{getLabel(group)}</summary><div className="mt-2 border-l border-white/10 pl-3">{group.items.map(item => <Link key={item.href} href={item.href} className={`block rounded-lg px-3 py-2 ${isCurrent(item.href) ? 'text-cyan-300' : 'text-white/75'}`}>{getLabel(item)}</Link>)}</div></details>}
        </div>)}
      </nav>}
    </header>
